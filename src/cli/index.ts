...
import I18nReplacer, {
  ...
  defaultEnglishStrategy,
} from '..';
import { availableLocales, EnglishStrategy } from '../types';
...
export async function cli() {
  program
    ...
    .addOption(
      new Option(
        '--englishStrategy <mode>',
        `english extraction strategy when localeToReplace is en-us, default is [${defaultEnglishStrategy}]`
      ).choices(['comment-only', 'balanced', 'aggressive'])
    )
  ...
  program.action(async () => {
    ...
    if (opts.englishStrategy) {
      opts.englishStrategy = opts.englishStrategy as EnglishStrategy;
    }
    if (opts.version) {
